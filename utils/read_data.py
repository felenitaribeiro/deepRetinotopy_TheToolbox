...
def read_nyu(path, hemisphere=None, sub_id=None,
             visual_mask_L=None, visual_mask_R=None,
             faces_L=None, faces_R=None, myelination=None, prediction=None, 
             stimulus = 'original'):
    """Read the data files and create a data object with attributes x, y, pos,
    ...
    if (hemisphere == 'Right' or hemisphere == 'RH' or hemisphere == 'right' or hemisphere == 'rh'):
        ...
        # 3D position of the right hemisphere vertices
        pos = torch.tensor((scipy.io.loadmat(
            osp.join('/scratch/project/recyle_dl/deepRetinotopy_TheToolbox/HCP/raw/converted/', 'mid_pos_R.mat'))['mid_pos_R'].reshape(
            (number_hemi_nodes, 3))[visual_mask_R == 1]),
            dtype=torch.float)
        ...
        # Functional
        if stimulus == 'original':
            R2_values = torch.tensor(np.array(nib.load(osp.join(path, 'freesurfer/' + sub_id + '/surf/' + sub_id +
                                                            '.fs_empirical_variance_explained_rh.func.gii')
                                                   ).agg_data()).reshape(number_hemi_nodes, -1)[visual_mask_R == 1], dtype=torch.float) * 100
            prob_values = torch.tensor(np.array(nib.load(osp.join(path, 'probability_maps/' + sub_id +
                                                            '.fs_empirical_pRFsize_rh_probability.func.gii')
                                                   ).agg_data()).reshape(number_hemi_nodes, -1)[visual_mask_R == 1], dtype=torch.float)
          
            if prediction == 'polarAngle':
                retinotopicMap_values = torch.tensor(np.array(nib.load(osp.join(path, 'freesurfer/' + sub_id + '/surf/' + sub_id +
                                                            '.fs_empirical_polarAngle_rh.func.gii')
                                                   ).agg_data()).reshape(number_hemi_nodes, -1)[visual_mask_R == 1], dtype=torch.float) 
            elif prediction == 'eccentricity':
                retinotopicMap_values = torch.tensor(np.array(nib.load(osp.join(path, 'freesurfer/' + sub_id + '/surf/' + sub_id +
                                                            '.fs_empirical_eccentricity_rh.func.gii')
                                                   ).agg_data()).reshape(number_hemi_nodes, -1)[visual_mask_R == 1], dtype=torch.float) 
            elif prediction == 'pRFsize':
                retinotopicMap_values = torch.tensor(np.array(nib.load(osp.join(path, 'freesurfer/' + sub_id + '/surf/' + sub_id +
                                                            '.fs_empirical_pRFsize_rh.func.gii')
                                                   ).agg_data()).reshape(number_hemi_nodes, -1)[visual_mask_R == 1], dtype=torch.float) 
        # Anatomical
        curvature = torch.tensor(np.array(nib.load(osp.join(path, 'freesurfer/' + sub_id + '/surf/' + sub_id +
                                                            '.curvature-midthickness.rh.32k_fs_LR.func.gii')
        ...
        noprob = np.isnan(prob_values)
        prob_values[noprob == 1] = 0
        ...
        # 3D position of the left hemisphere vertices
        pos = torch.tensor((scipy.io.loadmat(
            osp.join('/scratch/project/recyle_dl/deepRetinotopy_TheToolbox/HCP/raw/converted/', 'mid_pos_L.mat'))['mid_pos_L'].reshape(
            (number_hemi_nodes, 3))[visual_mask_L == 1]),
            dtype=torch.float)
        ...
        if stimulus == 'original':
            R2_values = torch.tensor(np.array(nib.load(osp.join(path, 'freesurfer/' + sub_id + '/surf/' + sub_id +
                                                            '.fs_empirical_variance_explained_lh.func.gii')
                                                   ).agg_data()).reshape(number_hemi_nodes, -1)[visual_mask_L == 1], dtype=torch.float) * 100
            prob_values = torch.tensor(np.array(nib.load(osp.join(path, 'probability_maps/' + sub_id +
                                                            '.fs_empirical_pRFsize_lh_probability.func.gii')
                                                    ).agg_data()).reshape(number_hemi_nodes, -1)[visual_mask_L == 1], dtype=torch.float)


            if prediction == 'polarAngle':
                retinotopicMap_values = torch.tensor(np.array(nib.load(osp.join(path, 'freesurfer/' + sub_id + '/surf/' + sub_id +
                                                            '.fs_empirical_polarAngle_lh.func.gii')
                                                   ).agg_data()).reshape(number_hemi_nodes, -1)[visual_mask_L == 1], dtype=torch.float)
            elif prediction == 'eccentricity':
                retinotopicMap_values = torch.tensor(np.array(nib.load(osp.join(path, 'freesurfer/' + sub_id + '/surf/' + sub_id +
                                                            '.fs_empirical_eccentricity_lh.func.gii')
                                                   ).agg_data()).reshape(number_hemi_nodes, -1)[visual_mask_L == 1], dtype=torch.float)
            elif prediction == 'pRFsize':
                retinotopicMap_values = torch.tensor(np.array(nib.load(osp.join(path, 'freesurfer/' + sub_id + '/surf/' + sub_id +
                                                            '.fs_empirical_pRFsize_lh.func.gii')
                                                   ).agg_data()).reshape(number_hemi_nodes, -1)[visual_mask_L == 1], dtype=torch.float)
