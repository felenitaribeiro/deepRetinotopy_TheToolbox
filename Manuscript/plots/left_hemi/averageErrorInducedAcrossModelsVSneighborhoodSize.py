...
for j in models:
    ...
    data_tmp = pd.DataFrame(
        {'Error': means, 'Model': max_neighborhood * ['model_' + str(j)],
         'Neighborhood': np.arange(1, max_neighborhood + 1),
         'Patch': max_neighborhood * ['inner']})
    data = data.append(data_tmp, ignore_index=True)

    data_tmp = pd.DataFrame({'Error': means_reversed,
                             'Model': max_neighborhood * ['model_' + str(j)],
                             'Neighborhood': np.arange(1,
                                                       max_neighborhood + 1),
                             'Patch': max_neighborhood * ['outer']})
    data = data.append(data_tmp, ignore_index=True)
