...
class Retinotopy(InMemoryDataset):
        ...
        if self.hemisphere == 'Left':
            if self.myelination == True:
                if self.prediction == 'eccentricity':
                    return [
                        'training_ecc_LH_myelincurv_ROI_ctePatch.pt',
                        'development_ecc_LH_myelincurv_ROI_ctePatch.pt',
                        'test_ecc_LH_myelincurv_ROI_ctePatch.pt']

                elif self.prediction == 'polarAngle':
                    return [
                        'training_PA_LH_myelincurv_ROI_ctePatch.pt',
                        'development_PA_LH_myelincurv_ROI_ctePatch.pt',
                        'test_PA_LH_myelincurv_ROI_ctePatch.pt']

                else:
                    return [
                        'training_pRFsize_LH_myelincurv_ROI_ctePatch.pt',
                        'development_pRFsize__LH_myelincurv_ROI_ctePatch.pt',
                        'test_pRFsize_LH_myelincurv_ROI_ctePatch.pt']
            else:
                if self.prediction == 'eccentricity':
                    return ['training_ecc_LH_ROI_ctePatch.pt',
                            'development_ecc_LH_ROI_ctePatch.pt',
                            'test_ecc_LH_ROI_ctePatch.pt']

                elif self.prediction == 'polarAngle':
                    return ['training_PA_LH_ROI_ctePatch.pt',
                            'development_PA_LH_ROI_ctePatch.pt',
                            'test_PA_LH_ROI_ctePatch.pt']
                else:
                    return [
                        'training_pRFsize_LH_ROI_ctePatch.pt',
                        'development_pRFsize_LH_ROI_ctePatch.pt',
                        'test_pRFsize_LH_ROI_ctePatch.pt']

        else:
            if self.myelination == True:
                if self.prediction == 'eccentricity':
                    return [
                        'training_ecc_RH_myelincurv_ROI_ctePatch.pt',
                        'development_ecc_RH_myelincurv_ROI_ctePatch.pt',
                        'test_ecc_RH_myelincurv_ROI_ctePatch.pt']

                elif self.prediction == 'polarAngle':
                    return [
                        'training_PA_RH_myelincurv_ROI_ctePatch.pt',
                        'development_PA_RH_myelincurv_ROI_ctePatch.pt',
                        'test_PA_RH_myelincurv_ROI_ctePatch.pt']

                else:
                    return [
                        'training_pRFsize_RH_myelincurv_ROI_ctePatch.pt',
                        'development_pRFsize_RH_myelincurv_ROI_ctePatch.pt',
                        'test_pRFsize_RH_myelincurv_ROI_ctePatch.pt']
            else:
                if self.prediction == 'eccentricity':
                    return ['training_ecc_RH_ROI_ctePatch.pt',
                            'development_ecc_RH_ROI_ctePatch.pt',
                            'test_ecc_RH_ROI_ctePatch.pt']

                elif self.prediction == 'polarAngle':
                    return ['training_PA_RH_ROI_ctePatch.pt',
                            'development_PA_RH_ROI_ctePatch.pt',
                            'test_PA_RH_ROI_ctePatch.pt']

                else:
                    return [
                        'training_pRFsize_RH_ROI_ctePatch.pt',
                        'development_pRFsize_RH_ROI_ctePatch.pt',
                        'test_pRFsize_RH_ROI_ctePatch.pt']
    ...
    def process(self):
        ...
        for i in range(0, self.n_examples):
            ...
            # Patch
            data.x[self.patch] = torch.ones(
                data.x[self.patch].shape) * torch.tensor(
