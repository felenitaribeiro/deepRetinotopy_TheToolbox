...
#SBATCH -o output_models-beta.txt
#SBATCH -e error_models-beta.txt
...
EP=400; BS=8; LR=0.005; SEEDS=1; CLIP=1.0; SWA_START=300
ROI=wholebrain


TAG="roi-${ROI}_ep${EP}_bs${BS}_lr${LR}_swa_myelincurv"
for hemisphere in LH RH;
do
    echo "=== Training eccentricity model ($ROI) for $hemisphere hemisphere ==="
    python -u ./train.py --path ./../HCP/ --path2list ./../HCP/subs.txt \
        --roi "$ROI" --prediction_type eccentricity --hemisphere "$hemisphere" \
        --n_epochs "$EP" --batch_size "$BS" --lr "$LR" \
        --grad_clip "$CLIP" --swa --swa_start "$SWA_START" --n_seeds "$SEEDS" --tag "$TAG" --num_features 2
done

for hemisphere in LH RH;
do
    echo "=== Training polar angle model ($ROI) for $hemisphere hemisphere ==="
    python -u ./train.py --path ./../HCP/ --path2list ./../HCP/subs.txt \
        --roi "$ROI" --prediction_type polarAngle --hemisphere "$hemisphere" \
        --n_epochs "$EP" --batch_size "$BS" --lr "$LR" \
        --grad_clip "$CLIP" --swa --swa_start "$SWA_START" --n_seeds "$SEEDS" --tag "$TAG" --num_features 2
done
