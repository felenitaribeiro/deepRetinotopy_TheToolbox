...
error = np.load('./../../stats/output/'
                'meanErrorVSnodes_dorsalEarlyVisualCortex_10neighbor_model3'
                '.npz')[
    'list']
...
fig, ax = plt.subplots()
ax.stem(np.arange(len(error)), error, linefmt='#4e4351', markerfmt=' ',
        bottom=-1)
# ax.axvspan(0, len(V1d_indeces) + len(V1v_indeces), facecolor='#7F508A',
# alpha = 0.5)
# ax.axvspan(len(V1d_indeces) + len(V1v_indeces), len(V1d_indeces) + len(
# V1v_indeces) + len(V2d_indeces) + len(V2v_indeces), facecolor='#0078be',
# alpha = 0.5)
# ax.axvspan(len(V1d_indeces) + len(V1v_indeces) + len(V2d_indeces) + len(
# V2v_indeces), len(V1d_indeces) + len(V1v_indeces) + len(V2d_indeces) +
# len(V2v_indeces) + len(V3d_indeces) + len(V3v_indeces),
# facecolor='#0098b5', alpha = 0.5)

# V1
ax.axvspan(0, len(V1d_indeces), facecolor='#7F508A', alpha=0.5)
ax.axvspan(len(V1d_indeces), len(V1d_indeces) + len(V1v_indeces),
           facecolor='#7F508A', alpha=0.4)
# V2
ax.axvspan(len(V1d_indeces) + len(V1v_indeces),
           len(V1d_indeces) + len(V1v_indeces) + len(V2d_indeces),
           facecolor='#6064aa', alpha=0.5)
ax.axvspan(len(V1d_indeces) + len(V1v_indeces) + len(V2d_indeces),
           len(V1d_indeces) + len(V1v_indeces) + len(V2d_indeces) + len(
               V2v_indeces), facecolor='#6064aa', alpha=0.4)
# V3
ax.axvspan(
    len(V1d_indeces) + len(V1v_indeces) + len(V2d_indeces) + len(V2v_indeces),
    len(V1d_indeces) + len(V1v_indeces) + len(V2d_indeces) + len(
        V2v_indeces) + len(V3d_indeces), facecolor='#0078be', alpha=0.5)
ax.axvspan(len(V1d_indeces) + len(V1v_indeces) + len(V2d_indeces) + len(
    V2v_indeces) + len(V3d_indeces),
           len(V1d_indeces) + len(V1v_indeces) + len(V2d_indeces) + len(
               V2v_indeces) + len(V3d_indeces) + len(V3v_indeces),
           facecolor='#0078be', alpha=0.4)
