...
for k in norm_value:
    train_dataset = Retinotopy(path, 'Train', transform=T.Cartesian(max_value=k),
    ...
    for i in range(5):
        ...
        torch.save(model.state_dict(),
                   osp.join(osp.dirname(osp.realpath(__file__)), 'norm_output',
                            'deepRetinotopy_PA_LH_model' + str(i+1) + '_'+ str(k)+'Norm.pt')) # Rename if RH
